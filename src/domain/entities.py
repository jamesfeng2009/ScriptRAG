...
class User(Base):
    ...
    # 关系
    screenplay_sessions = relationship("ScreenplaySession", back_populates="user")
...
class QuotaUsage(Base):
    ...
    id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid4)
    user_id = Column(PG_UUID(as_uuid=True), nullable=True)
    resource_type = Column(String(100), nullable=False)  # 'llm_tokens', 'api_calls', 'storage_mb'
    usage_amount = Column(Float, nullable=False)
    quota_limit = Column(Float)
    period_start = Column(DateTime(timezone=True), nullable=False)
    period_end = Column(DateTime(timezone=True), nullable=False)
    usage_metadata = Column(JSON, default={})  # 重命名避免冲突
