# RAG Screenplay Multi-Agent System

基于 RAG（检索增强生成）的剧本生成多智能体系统，使用 LangGraph 编排六个专门的智能体协同工作，生成基于实际项目代码和文档的技术剧本。

## 核心特性

- **多智能体协作架构**: 六个专门智能体分工协作（规划器、导航器、导演、转向管理器、编剧、编译器）
- **Agentic RAG 架构**: 智能意图解析 → 多源检索 → 质量评估 → 自适应循环
  - IntentParserAgent: 使用 LLM 解析查询意图，提取关键词和推荐数据源
  - QualityEvaluatorAgent: 多维度评估检索结果（相关性、完整性、新鲜度）
  - Agentic Loop: 像人一样思考和调整，迭代直到满意
- **RAG 混合检索**: 结合向量搜索和关键词匹配的智能检索
- **多 LLM 提供商支持**: OpenAI、通义千问、MiniMax、智谱 GLM，支持自动回退
- **动态 Skill 系统**: 六种生成风格（标准教程、警告模式、可视化类比、研究模式、轻松风格、回退摘要）
- **Task Stack 任务管理**: 支持递归任务分解和嵌套任务上下文管理，可配置最大深度
- **事实检查机制**: 幻觉检测和防御，确保内容基于真实来源
- **优雅降级**: 完善的错误处理和超时保护机制

## 系统架构

### 整体架构图

```
┌─────────────────────────────────────────────────────────────┐
│                      用户输入                                  │
│                  (主题 + 项目上下文)                           │
└────────────────────┬────────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────┐
│                  LangGraph 状态机                             │
│                 (WorkflowOrchestrator)                       │
└─────────────────────────────────────────────────────────────┘
                     │
        ┌────────────┼────────────┐
        ▼            ▼            ▼
   ┌────────┐  ┌────────┐  ┌────────┐
   │规划器  │  │意图解析 │  │导演    │
   │Planner │  │Intent  │  │Director│
   │        │  │Parser  │  │        │
   └────────┘  └────────┘  └────────┘
        │            │            │
        │            ▼            │
        │      ┌────────┐         │
        │      │导航器  │         │
        │      │Navigat│         │
        │      │or     │         │
        │      └────────┘         │
        │            │            │
        ▼            ▼            ▼
   ┌────────┐  ┌────────┐  ┌────────┐
   │转向管理│  │质量评估│  │编剧    │
   │Pivot   │  │Quality │  │Writer  │
   │Manager │  │Evaluat.│  └────────┘
   └────────┘  └────────┘       │
              │                 ▼
              │           ┌────────┐
              │           │事实检查│
              │           │Fact    │
              │           │Checker │
              │           └────────┘
              │                │
              └────────────────┼──────┐
                               ▼
                         ┌────────┐
                         │编译器  │
                         │Compiler│
                         └────────┘
                               │
                               ▼
                        ┌──────────┐
                        │最终剧本  │
                        └──────────┘
```

### 分层架构

系统采用企业级分层架构设计，确保可扩展性、可维护性和商业化部署：

```
┌─────────────────────────────────────────────────────────────┐
│  表示层 (Presentation Layer)                                  │
│  - CLI 接口、REST API、Web UI、监控仪表板                      │
└─────────────────────────────────────────────────────────────┘
                          │
┌─────────────────────────────────────────────────────────────┐
│  应用层 (Application Layer)                                   │
│  - LangGraph 工作流编排、智能体协调器、任务调度、会话管理      │
└─────────────────────────────────────────────────────────────┘
                          │
┌─────────────────────────────────────────────────────────────┐
│  业务逻辑层 (Business Logic Layer)                            │
│  - 六大核心智能体、Skill 管理器、事实检查器、冲突解决器        │
└─────────────────────────────────────────────────────────────┘
                          │
┌─────────────────────────────────────────────────────────────┐
│  服务层 (Service Layer)                                       │
│  - LLM 服务、向量数据库服务、代码解析服务、摘要服务            │
└─────────────────────────────────────────────────────────────┘
                          │
┌─────────────────────────────────────────────────────────────┐
│  数据访问层 (Data Access Layer)                               │
│  - PostgreSQL + pgvector、Redis 缓存、MinIO 对象存储          │
└─────────────────────────────────────────────────────────────┘
                          │
┌─────────────────────────────────────────────────────────────┐
│  基础设施层 (Infrastructure Layer)                            │
│  - 日志系统、监控追踪、错误处理、性能优化                      │
└─────────────────────────────────────────────────────────────┘
```

### 目录结构

```
src/
├── presentation/          # 表示层：CLI、API 接口
├── application/          # 应用层：工作流编排
│   └── orchestrator.py   # LangGraph 状态机管理
├── domain/              # 业务逻辑层：智能体、模型、技能
│   ├── agents/          # 六大智能体实现
│   │   ├── task_aware_planner.py  # 规划器：生成初始大纲（支持 Task Stack）
│   │   ├── navigator.py        # 导航器：RAG 检索
│   │   ├── director.py         # 导演：评估和决策
│   │   ├── pivot_manager.py    # 转向管理器：修正大纲
│   │   ├── writer.py           # 编剧：生成剧本片段
│   │   ├── compiler.py         # 编译器：整合最终剧本
│   │   ├── fact_checker.py     # 事实检查器：验证内容
│   │   └── retry_protection.py # 重试保护：防止无限循环
│   ├── task_stack.py      # Task Stack：嵌套任务管理
│   ├── models.py        # 数据模型（GlobalState、OutlineStep 等）
│   └── skills.py        # Skill 系统配置
├── services/            # 服务层：外部服务集成
│   ├── llm/            # LLM 服务
│   │   ├── service.py          # 统一 LLM 服务
│   │   ├── adapter.py          # LLM 适配器接口
│   │   ├── openai_adapter.py   # OpenAI 适配器
│   │   ├── qwen_adapter.py     # 通义千问适配器
│   │   ├── minimax_adapter.py  # MiniMax 适配器
│   │   └── glm_adapter.py      # 智谱 GLM 适配器
│   ├── database/       # 数据库服务
│   │   ├── postgres.py         # PostgreSQL 连接
│   │   ├── vector_db.py        # 向量数据库服务
│   │   ├── redis_cache.py      # Redis 缓存
│   │   └── logging_db.py       # 日志持久化
│   ├── parser/         # 代码解析服务
│   │   └── tree_sitter_parser.py  # Tree-sitter 解析器
│   ├── retrieval_service.py    # 检索服务
│   └── summarization_service.py # 摘要服务
└── infrastructure/      # 基础设施层：横切关注点
    ├── logging.py       # 日志系统
    ├── monitoring.py    # 监控追踪
    └── error_handler.py # 错误处理和优雅降级
```

## 核心实现思路

### 1. 导演驱动循环（Director-Driven Loop）

系统采用独特的"导演驱动循环"模式，而非简单的线性流程：

```
用户输入 → 规划器生成大纲 → 循环处理每个步骤：
    ├─ 导航器检索相关内容
    ├─ 导演评估内容质量
    │   ├─ 发现冲突？→ 转向管理器修正大纲 → 重新检索
    │   ├─ 内容复杂？→ 切换 Skill（如可视化类比）
    │   └─ 批准？→ 编剧生成片段
    ├─ 事实检查器验证片段
    │   ├─ 有幻觉？→ 重新生成
    │   └─ 有效？→ 继续下一步
    └─ 所有步骤完成 → 编译器整合最终剧本
```

**核心优势**：
- **自适应调整**：导演可以根据检索内容动态调整策略
- **质量保证**：多重检查机制（冲突检测、事实验证）
- **优雅降级**：遇到问题时自动切换策略而非失败

### 2. RAG 混合检索策略

导航器智能体实现了创新的混合检索方法：

```python
# 向量搜索（语义相似度）
vector_results = search_by_embedding(query_embedding, top_k=5)

# 关键词搜索（敏感标记）
keyword_results = search_by_markers(
    markers=["@deprecated", "FIXME", "TODO", "Security"]
)

# 加权合并
final_results = merge_with_weights(
    vector_results, weight=0.6,
    keyword_results, weight=0.4,
    boost_factor=1.5  # 敏感标记命中时加权
)
```

**技术栈**：
- **初期**：PostgreSQL 17 + pgvector（统一数据库，降低运维复杂度）
- **扩展**：当向量数据量 > 100 万或 QPS > 100 时，迁移到 Milvus

### 3. 动态 Skill 系统

六种生成风格适应不同场景：

| Skill | 使用场景 | 语调 |
|-------|---------|------|
| `standard_tutorial` | 标准教程 | 专业、正式 |
| `warning_mode` | 废弃功能、安全问题 | 警示、谨慎 |
| `visualization_analogy` | 复杂概念 | 生动、易懂 |
| `research_mode` | 信息不足 | 探索性 |
| `meme_style` | 轻松内容 | 幽默、随意 |
| `fallback_summary` | 降级模式 | 中性、简洁 |

**Skill 兼容性规则**：
```
standard_tutorial ↔ visualization_analogy
standard_tutorial ↔ warning_mode
warning_mode ↔ research_mode
visualization_analogy ↔ meme_style
meme_style ↔ fallback_summary
fallback_summary ↔ standard_tutorial
```

### 4. 多 LLM 提供商架构

统一适配器接口 + 自动回退机制：

```python
# 配置示例
LLM_CONFIG = {
    "active_provider": "openai",
    "fallback_providers": ["qwen", "glm"],
    "model_mappings": {
        "openai": {
            "high_performance": "gpt-4o",      # 规划器、导演
            "lightweight": "gpt-4o-mini",      # 编剧、编译器
            "embedding": "text-embedding-3-large"
        }
    }
}
```

**自动回退流程**：
1. 主提供商调用失败（速率限制、API 错误）
2. 使用指数退避重试（最多 3 次）
3. 仍失败？自动切换到回退提供商
4. 记录所有切换事件到日志

### 5. 事实检查和幻觉防御

三层防御机制：

```
1. 来源追踪：所有陈述必须标注来源
   [来源: src/utils/helper.py]

2. LLM 验证：使用高性能模型交叉验证
   - 检测不存在的函数、类、参数
   - 对比片段与源文档

3. 启发式规则：正则表达式检测明显错误
   - 函数定义：def function_name(
   - 函数调用：`function_name()`
   - 类引用：`ClassName` 类
```

**发现幻觉时的处理**：
- 移除无效片段
- 增加重试计数器
- 重新生成（最多 3 次）
- 超过限制？强制降级（跳过步骤）

### 6. 错误处理和优雅降级

完善的错误处理策略：

| 错误类型 | 处理策略 | 回退行为 |
|---------|---------|---------|
| PostgreSQL 连接失败 | 回退到仅关键词搜索 | 返回空结果 |
| 嵌入生成失败 | 跳过向量搜索 | 使用关键词搜索 |
| LLM 速率限制 | 指数退避重试 | 切换提供商 |
| 事实检查器失败 | 记录警告 | 假设片段有效 |
| 摘要器失败 | 使用截断 | 保留前 10000 字符 |
| 编剧失败 | 切换到 fallback_summary | 生成简单概述 |
| 智能体超时 | 跳过步骤 | 继续下一步 |

**超时配置**：
- 规划器/导航器/导演/编剧/事实检查器/编译器：60 秒
- 转向管理器：30 秒
- 重试保护：10 秒

### 7. 基于属性的测试（Property-Based Testing）

使用 Hypothesis 框架验证系统正确性：

```python
# 属性示例：状态修改一致性
@given(state=shared_state_strategy())
def test_state_consistency(state):
    """任何智能体修改状态后，后续智能体应能看到修改"""
    # 生成 100+ 随机测试用例
    assert all_agents_see_same_state(state)
```

**27 个核心属性 + 3 个边界情况**：
- 状态一致性、混合检索完整性、幻觉检测
- 重试限制执行、Skill 兼容性、优雅降级
- 空检索无幻觉、最大重试边界、Token 阈值边界

### 8. Task Stack 任务管理

规划器智能体支持可选的 Task Stack 功能，用于管理嵌套任务和递归任务分解：

```python
# 启用 Task Stack
factory = NodeFactory(
    llm_service=llm_service,
    retrieval_service=retrieval_service,
    parser_service=parser_service,
    summarization_service=summarization_service,
    use_task_stack=True,      # 启用 Task Stack
    max_task_depth=5         # 最大嵌套深度（默认 3）
)

# 或通过 Orchestrator 配置
orchestrator = WorkflowOrchestrator(
    llm_service=llm_service,
    enable_task_stack=True,
    max_task_depth=5
)
```

**Task Stack 核心功能**：
- **嵌套任务管理**：支持递归分解复杂任务为子任务
- **上下文隔离**：每个任务拥有独立的上下文数据
- **深度限制**：可配置的最大嵌套深度防止无限递归
- **任务追溯**：支持获取当前任务、父任务、任务深度等信息

**TaskStackManager API**：
```python
# 创建子任务
state = planner.create_subtask(state, subtask_data)

# 完成子任务（恢复父任务上下文）
state, popped_context = planner.complete_subtask(state)

# 获取当前任务
current_task = planner.get_current_task(state)

# 检查是否有父任务
has_parent = planner.has_parent_task(state)

# 获取当前深度
depth = planner.get_task_depth(state)
```

**任务上下文结构**：
```python
TaskContext {
    task_id: str           # 唯一任务标识
    parent_id: str | None  # 父任务 ID
    depth: int            # 当前深度（0 为主任务）
    creation_timestamp: datetime
    task_data: Dict[str, Any]  # 任务相关数据
}
```

## 技术栈

## 技术栈

### 核心框架
- **编排引擎**: LangGraph（Python）- 状态机管理
- **数据模型**: Pydantic - 类型安全的数据验证
- **异步处理**: asyncio - 高性能异步 I/O

### LLM 和嵌入
- **LLM 提供商**: 
  - OpenAI（GPT-4o、GPT-4o-mini）
  - 通义千问（qwen-max、qwen-turbo）
  - MiniMax（abab6.5-chat、abab5.5-chat）
  - 智谱 GLM（glm-4、glm-3-turbo）
- **嵌入模型**: 
  - OpenAI text-embedding-3-large
  - 通义千问 text-embedding-v2
  - MiniMax embo-01
  - 智谱 embedding-2

### 数据存储（渐进式架构）

**初期架构（MVP 阶段）**：
- **关系型数据库**: PostgreSQL 17 + pgvector（统一数据库）
- **缓存**: Redis 7+（单机或主从模式）
- **对象存储**: MinIO（单机）
- **消息队列**: RabbitMQ（单机）

**扩展架构（规模化阶段）**：
- **向量数据库**: Milvus（分布式集群）
- **关系型数据库**: PostgreSQL 集群（主从复制、读写分离）
- **缓存**: Redis Cluster（高可用集群）
- **对象存储**: MinIO 集群或云存储（S3/OSS/COS）

**迁移触发条件**：
- 向量数据量 > 100 万条
- 向量搜索 QPS > 100
- P99 延迟 > 500ms
- 存储大小 > 100GB

### 基础设施
- **代码解析**: Tree-sitter（多语言支持）
- **监控**: Prometheus + Grafana
- **日志**: ELK Stack（Elasticsearch + Logstash + Kibana）
- **追踪**: Jaeger / OpenTelemetry
- **容器化**: Docker + Docker Compose
- **编排**: Kubernetes（生产环境）
- **测试**: Pytest + Hypothesis（属性测试）

## 快速开始

## 快速开始

### 1. 环境要求

- Python 3.10+
- PostgreSQL 17（带 pgvector 扩展）
- Redis 7+
- 至少一个 LLM 提供商的 API 密钥

### 2. 安装依赖

**使用 Poetry（推荐）**：

```bash
# 安装 Poetry
curl -sSL https://install.python-poetry.org | python3 -

# 安装项目依赖
poetry install
```

**使用 pip**：

```bash
pip install -r requirements.txt
```

### 3. 配置环境

1. 复制环境变量模板：
```bash
cp .env.example .env
```

2. 编辑 `.env` 文件，配置 API 密钥和数据库连接：
```bash
# LLM 提供商配置
OPENAI_API_KEY=your_openai_key
QWEN_API_KEY=your_qwen_key
MINIMAX_API_KEY=your_minimax_key
GLM_API_KEY=your_glm_key

# 数据库配置
DATABASE_URL=postgresql://user:password@localhost:5432/screenplay_db
REDIS_URL=redis://localhost:6379/0
```

3. 自定义 `config.yaml` 进行高级配置（可选）

### 4. 数据库初始化

```bash
# 创建数据库
createdb screenplay_db

# 启用 pgvector 扩展
psql -d screenplay_db -c "CREATE EXTENSION vector;"

# 运行初始化脚本
psql -d screenplay_db -f scripts/init_db.sql
psql -d screenplay_db -f scripts/create_core_tables.sql
psql -d screenplay_db -f scripts/create_vector_tables.sql
psql -d screenplay_db -f scripts/create_log_tables.sql
psql -d screenplay_db -f scripts/create_functions.sql
psql -d screenplay_db -f scripts/performance_optimization.sql

# 验证设置
python scripts/verify_setup.py
```

### 5. 使用 Docker Compose（推荐用于开发）

```bash
# 启动所有服务（PostgreSQL、Redis、MinIO）
docker-compose up -d

# 查看日志
docker-compose logs -f

# 停止服务
docker-compose down
```

## 使用方法

### CLI 使用（开发中）

```bash
# 基本用法
python -m src.presentation.cli \
    --topic "用户认证系统实现" \
    --context "基于 FastAPI 和 JWT 的认证系统"

# 指定 Skill
python -m src.presentation.cli \
    --topic "复杂的微服务架构" \
    --skill visualization_analogy

# 指定 LLM 提供商
python -m src.presentation.cli \
    --topic "数据库优化策略" \
    --provider qwen
```

### API 使用（开发中）

```bash
# 启动 API 服务器
python -m src.presentation.api

# 发送请求
curl -X POST http://localhost:8000/api/v1/generate \
  -H "Content-Type: application/json" \
  -d '{
    "topic": "Redis 缓存策略",
    "project_context": "高并发电商系统"
  }'
```

### Python API

```python
from src.application.orchestrator import WorkflowOrchestrator
from src.services.llm.service import LLMService
from src.services.retrieval_service import RetrievalService
from src.domain.models import SharedState

# 初始化服务
llm_service = LLMService.from_yaml("config/llm_providers.yaml")
retrieval_service = RetrievalService(...)

# 创建编排器
orchestrator = WorkflowOrchestrator(
    llm_service=llm_service,
    retrieval_service=retrieval_service,
)

# 执行工作流
initial_state = SharedState(
    user_topic="分布式事务处理",
    project_context="微服务架构"
)

final_screenplay = await orchestrator.execute(initial_state)
print(final_screenplay)
```

## 开发指南

### 运行测试

```bash
# 运行所有测试
pytest

# 单元测试
pytest tests/unit/ -v

# 属性测试（基于 Hypothesis）
pytest tests/property/ -v

# 集成测试
pytest tests/integration/ -v

# 带覆盖率报告
pytest --cov=src --cov-report=html --cov-report=term

# 运行特定测试
pytest tests/property/test_graceful_component_failure.py -v
```

### 代码质量

```bash
# 格式化代码
black src/ tests/

# 代码检查
ruff check src/ tests/

# 类型检查
mypy src/

# 运行所有质量检查
make lint  # 如果有 Makefile
```

### 项目结构说明

详细的项目结构说明请参考：
- [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) - 完整的目录结构
- [src/services/README.md](src/services/README.md) - 服务层设计
- [src/domain/README_SKILLS.md](src/domain/README_SKILLS.md) - Skill 系统详解

## 性能基准

系统性能目标：

| 指标 | 目标值 | 说明 |
|------|--------|------|
| 单步骤处理时间 | < 5s | 包括检索、评估、生成 |
| 完整剧本生成时间 | < 60s | 10 步大纲的完整流程 |
| RAG 检索延迟 | < 500ms | 向量搜索 + 关键词搜索 |
| LLM 响应延迟 | < 3s | 单次 LLM 调用 |
| 系统吞吐量 | > 100 req/min | 并发请求处理能力 |
| API 响应时间（P95） | < 2s | 95% 的请求在 2 秒内完成 |

## 商业化特性

### 监控和可观测性
- Prometheus 指标收集
- Grafana 仪表板
- 分布式追踪（Jaeger）
- 集中式日志（ELK Stack）

## 部署

### 单机部署（开发/测试）

```bash
# 使用 Docker Compose
docker-compose up -d

# 查看服务状态
docker-compose ps

# 查看日志
docker-compose logs -f screenplay-app
```

### 分布式部署（生产环境）

```bash
# 使用 Kubernetes
kubectl apply -f k8s/namespace.yaml
kubectl apply -f k8s/configmap.yaml
kubectl apply -f k8s/secrets.yaml
kubectl apply -f k8s/deployment.yaml
kubectl apply -f k8s/service.yaml
kubectl apply -f k8s/ingress.yaml

# 查看部署状态
kubectl get pods -n screenplay-system
kubectl get svc -n screenplay-system
```

详细部署指南请参考 [部署文档](docs/DEPLOYMENT.md)（待添加）。

## 常见问题

### Q: 如何切换 LLM 提供商？

编辑 `config/llm_providers.yaml`：
```yaml
active_provider: "qwen"  # 改为 qwen、minimax 或 glm
fallback_providers: ["openai", "glm"]
```

### Q: 如何添加新的 Skill？

1. 在 `src/domain/skills.py` 中添加 Skill 定义
2. 在 `src/domain/agents/writer.py` 中添加 Skill 提示模板
3. 更新 Skill 兼容性规则
4. 编写单元测试

### Q: 向量数据库何时需要迁移到 Milvus？

当满足以下任一条件时：
- 向量数据量 > 100 万条
- 向量搜索 QPS > 100
- P99 延迟 > 500ms
- 存储大小 > 100GB

### Q: 如何调试智能体执行流程？

1. 启用详细日志：
```python
import logging
logging.basicConfig(level=logging.DEBUG)
```

2. 查看执行日志：
```python
print(state.execution_log)
```

3. 使用 LangGraph 可视化工具（开发中）

## 贡献指南

欢迎贡献！请遵循以下步骤：

1. Fork 本仓库
2. 创建特性分支（`git checkout -b feature/AmazingFeature`）
3. 提交更改（`git commit -m 'Add some AmazingFeature'`）
4. 推送到分支（`git push origin feature/AmazingFeature`）
5. 开启 Pull Request

请确保：
- 所有测试通过
- 代码符合 PEP 8 规范
- 添加必要的文档和注释
- 更新相关的 README 和文档

## 项目状态

本项目正在积极开发中。当前进度：

- ✅ 核心架构设计
- ✅ 六大智能体实现
- ✅ RAG 混合检索
- ✅ 多 LLM 提供商支持
- ✅ Skill 系统
- ✅ 事实检查机制
- ✅ 错误处理和优雅降级
- ✅ 属性测试（27 个属性 + 3 个边界情况）
- 🚧 CLI 和 API 接口（开发中）
- 🚧 监控和可观测性（开发中）
- 📋 Web UI（计划中）
- 📋 Milvus 迁移工具（计划中）

详细进度请查看 [任务列表](.kiro/specs/rag-screenplay-multi-agent/tasks.md)。

## 许可证

[MIT License](LICENSE)

## 联系方式

- 项目主页：[GitHub Repository URL]
- 问题反馈：[GitHub Issues URL]
- 文档：[Documentation URL]

## 致谢

本项目使用了以下优秀的开源项目：
- [LangGraph](https://github.com/langchain-ai/langgraph) - 状态机编排
- [Hypothesis](https://hypothesis.readthedocs.io/) - 属性测试框架
- [pgvector](https://github.com/pgvector/pgvector) - PostgreSQL 向量扩展
- [Tree-sitter](https://tree-sitter.github.io/) - 代码解析
- [FastAPI](https://fastapi.tiangolo.com/) - Web 框架（计划使用）

感谢所有贡献者的辛勤工作！
